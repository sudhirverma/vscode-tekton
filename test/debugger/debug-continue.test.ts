...
import { showDebugContinue } from '../../src/debugger/debug-continue';
...
suite('debug/TaskRunContinue', () => {
  ...
  suite('Debug Continue TaskRun', () => {

    test('return null if no taskRun found', async () => {
      const result = await showDebugContinue(null);
      expect(result).equals(null);
    });

    test('Debug and continue taskRun', async () => {
      sandbox.stub(debugSessions, 'get').returns({containerName: 'test-task-run', podName: 'test', namespace: 'test'});
      await showDebugContinue(taskRunNode);
      expect(cliExecStub).calledOnceWith(Command.debugContinue('test-task-run', 'test', 'test'));
    });
    ...
    test('check if any error appear from debugger', async () => {
      ...
      const result = await showDebugContinue(taskRunNode);
