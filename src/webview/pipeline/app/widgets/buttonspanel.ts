...
import { addItemInWorkspace, collectParameterData, collectResourceData, collectServiceAccountData, collectTriggerData, collectWorkspaceData, createNewPipelineResource, createPVC, createNewVCT, getRandomChars, removePvcName, storePvcName } from '../utils/resource';
  ...
  storeVolumeClaimTemplate(data: unknown[] | NodeListOf<Element>): void {
    if (data.length !== 0) {
      data.forEach(val => {
        ...
        const size = val.querySelectorAll('[id^=Size-for-PVC-Storage]')[0].value;
        const inputSize = val.querySelectorAll('[id^=size-for-pvc-create-webview]')[0].value;
        createNewVCT(resourceName, accessMode, size, inputSize, this.initialValue);
      })
    }
  }
