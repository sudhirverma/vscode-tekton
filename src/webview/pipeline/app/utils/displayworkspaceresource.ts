...
function dropdownForWorkspaceType(event: Node & ParentNode | HTMLSelectElement, editId: string, sectionId: string, select: HTMLSelectElement, trigger: Trigger, initialValue: PipelineStart, index?: number, pipelineRunWorkspaceVCT?: unknown): void {
  ...
  if (pipelineRunVCT) {
    console.log('jkjdlkjjjfdsjdfljldsjlfdsjlkfdsjlkdsjlkfdsjlsfdjlkfdsj');
    ...
    if (option) {
      ...
      console.log(pipelineRunWorkspaceVCT?.[event?.firstElementChild.id]);
...
export function createVCT(event: Node & ParentNode, select: HTMLSelectElement, initialValue: PipelineStart, pipelineRunVCT?: VCT): void {
  if (select[select.selectedIndex].id === volumeClaimTemplateID) {
...
export function createVolumeClaim(event: Node & ParentNode, input: EditItem, newDivClass: string, initialValue: PipelineStart, pipelineRunVCT?: VCT): void {
  ...
  if (storage) {
    ...
    binary = storage.replace(/[0-9]/g, '');
  }
  console.log(numberInput);
  console.log(binary);
  console.log(accessMode);
  const selectAccessMode = new SelectWidget('editor-select-box-item-select-a-key', null, 'editor-select-box-item', initialValue, 'Access-Mode-for-Pvc').selectAccessMode((accessMode?.length !== 0) ? accessMode[0] : null);
  ...
  const selectSize = new SelectWidget('editor-select-box-item-select-a-key', null, 'size-select-box-item', initialValue, 'Size-for-PVC-Storage').selectSize(binary);
